...
**Status:** implementado — `LLMAdapter` expõe `screen_risk`, enum `RiskScreeningResult` no domínio,
bloco de segurança inserido deterministicamente via `asset_loader.py` e rodapé fixo de RF18
incondicionalmente inserido em todas as saídas geradas (`analysis_markdown`, `report_markdown`,
`trend_markdown`). Prompts sincronizados e travados contra deriva via `test_prompt_sync.py` (T7.4).
